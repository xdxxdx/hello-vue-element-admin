<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="listQuery.status" placeholder="轮播状态" clearable style="width: 90px" class="filter-item">
    ...
    >
      ...
      <el-table-column label="轮播图片" min-width="150px">
        <template slot-scope="{row}">
          <img :src=" row.photoSrc | showImage" width="60" height="40" @click="handleUpdate(row)">
          <!-- <span class="link-type" @click="handleUpdate(row)">{{ row.photoSrc }}</span> -->
        </template>
      </el-table-column>
      <el-table-column label="跳转地址" min-width="150px">
        <template slot-scope="{row}">
          <span class="link-type">{{ row.jumpUrl }}</span>
        </template>
      </el-table-column>
      <el-table-column label="轮播状态" min-width="150px">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="80px" style="width: 400px;  margin-left:50px;">
        <el-form-item label="轮播图片" prop="photoSrc">
          <el-upload
            class="avatar-uploader"
            action="/ele/admin/singleUpload/carousel"
            name="upload"
...
<script>
import { fetchList, create, update, setStatus, hidden } from '@/api/carousel'
